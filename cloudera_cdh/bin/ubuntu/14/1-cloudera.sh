...
echo "Installing MySQL(v5.6)..."
sudo echo 'mysql-server-5.6 mysql-server/root_password password SCALE42secretly' | sudo debconf-set-selections 
sudo echo 'mysql-server-5.6 mysql-server/root_password_again password SCALE42secretly' | sudo debconf-set-selections
sudo apt-get install mysql-server-5.6 -y

echo "MySQL(v5.6) installation has been successfully completed !!!"