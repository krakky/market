sudo sed -i "s/port .*/port 7000/g" /etc/redis.conf
...
sudo sed -i "s/# cluster-node-timeout 5000/cluster-node-timeout 5000/g" /etc/redis.conf
sudo sed -i "s/# cluster-config-file nodes-6379.conf/cluster-config-file nodes.conf/g" /etc/redis.conf
