...
sudo sed -i "s|cluster_name: 'Test Cluster'|cluster_name: 'ACM-deployed-CLUSTER'|g" /etc/cassandra/cassandra.yaml

# >>> 2 <<<
echo "Assigning the <seeds> parameter with the ENTIRE_CLUSTER list !"
sudo sed -i "s|- seeds: \"127.0.0.1\"|- seeds: \"$ENTIRE_CLUSTER\"|g" /etc/cassandra/cassandra.yaml

# >>> 3 <<<
echo "Changing <listen_address> parameter from LOCALHOST with the variable {{ SELF.IP }} passed argument [$CURRENT_IP] !"
sudo sed -i "s|listen_address: localhost|listen_address: $CURRENT_IP|g" /etc/cassandra/cassandra.yaml

# >>> 4 <<<
echo "Changing <rpc_address> parameter from LOCALHOST with the variable {{ SELF.IP }} passed argument [$CURRENT_IP] !"
sudo sed -i "s|rpc_address: localhost|rpc_address: $CURRENT_IP|g" /etc/cassandra/cassandra.yaml
...
# >>> 6 <<<
echo "Appending a single YAML configuration(line) of <auto_bootstrap: true> to /etc/cassandra/cassandra.yaml !"
...
#START FOR UBUNTU-16
echo "Enabling and starting CASSANDRA service -> Ubuntu 16!"
