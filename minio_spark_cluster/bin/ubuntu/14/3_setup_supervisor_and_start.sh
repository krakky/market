...
sudo echo "command=minio server ~/minio_data --config-dir /etc/minio/config" >> /etc/supervisor/conf.d/minio-cluster.conf
sudo echo "directory=/usr/local/bin/" >> /etc/supervisor/conf.d/minio-cluster.conf
